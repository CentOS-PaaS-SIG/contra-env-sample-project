...
def run_info():
    ...
    if debug_result_id is not None:
        ...
        if debug_result.get('Result'):
            ...
            build_configs = test_run_info.get('build_config_name_files')
    ...
    return {
        ...
        'oc_bin': oc_bin,
        'build_configs': build_configs.values() if build_configs else [],
    }
...
def test_buildconfigs(run_info):
    ...
    assert(oc_result and 'pipeline-sample-container ' in oc_result)
...
def test_builds(run_info):
    ...
    pipeline_sample_container_success = False
    ...
    if oc_bin is not None:
        ...
        for line in oc_result.splitlines():
            ...
            if 'pipeline-sample-container-' in line and 'Complete' in line:
                pipeline_sample_container_success = True
    ...
    assert(pipeline_sample_container_success)
